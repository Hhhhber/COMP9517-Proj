...
def load_model(ckpt_path: str, device: torch.device):
    ...
    if num_classes is None:
        raise ValueError("can't infer num_classes from weights，please give the order: --num_classes")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--images_dir", required=True, help="img dir that need to follow, e.g. data/test/images")
    ap.add_argument("--weights", required=True)
    ap.add_argument("--out", required=True)
    ap.add_argument("--conf", type=float, default=0.6)
    ap.add_argument("--device", default="cuda")
    ap.add_argument("--num_classes", type=int, default=None, help="can point the class by yourself")
    a = ap.parse_args()
    main(a)
